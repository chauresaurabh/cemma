...
if($spassword eq $passworddb)
{
	...
	$username=param("tag");
	$sth = $db->prepare("select Passwd, Class, Email, FirstName, LastName, Telephone, Dept, Advisor, GradYear, GradTerm, ActiveUser, LastStatusUpdate  from user where UserName = '$username'");
	$sth->execute();
	($password, $class, $email, $firstname, $lastname, $tel, $dept, $advisor, $gradyear, $gradterm, $activeUser, $LastStatusUpdate) = $sth->fetchrow_array();
	$db->disconnect;	
	$sth->finish();
	...
	print "  <form method=post action=\"ModifyUsersInArchivedData.pl\" onsubmit=\"return CheckBeforeSubmit()\" name=\"newuser\">\n";
	...
	print "				<br/><input type=\"checkbox\" name = \"activate1\" value='active'> Re-Activate User\n";
	print "				<br/><input type=\"hidden\" name = \"lastupdatestatus1\" value='$LastStatusUpdate'>Last Status Update: $LastStatusUpdate";
	print "		   </td>\n";
	print "</tr>\n";
	print "<tr>\n";
