...
{ 
	...
	$sth = $db->prepare("select UserName, Passwd, Class,ActiveUser,Email,ResponseEmail,UserClass from user where UserName = \"$login\" ");
	$sth->execute();
	($UserName, $passworddb, $Class, $ActiveUser,$Email,$ResponseEmail,$UserClass) = $sth->fetchrow_array();
	$sth->finish();
	$db->disconnect();

if($ActiveUser eq "inactive")
{
#		$url = "../loginerror_activateuser.php?type=inactive&email=$Email";
		$url = "http://cemma-usc.net/schedule/cgi-bin/UpdateStatus.php?email=$Email&first=true&showlogin=true";
		print "Location: $url\n\n";
	#	$url = "http://www.usc.edu/dept/CEMMA/sign_up.htm";
	#	print "Location: $url\n\n";
		exit;
...
else
{
	...
	{
	#$url = "../loginerror_activateuser.php?type=pending&email=$Email";
		$url = "http://cemma-usc.net/schedule/cgi-bin/UpdateStatus.php?email=$Email&first=true&showlogin=true&newsystem=true";
		print "Location: $url\n\n";
		exit;

	}

	elsif($passworddb eq "") # Login error. (No such login name)
	{	
		$url = "../loginerr.htm?error=1";
		print redirect( -uri => $url);
		exit;
	}
	elsif ($UserName ne $login)
	{
		$url = "../loginerr.htm?error=2";
		print "Location: $url\n\n";
		exit;
	}	  
	elsif($passworddb eq $password & $UserClass ne 4) # Verify administrator's identity
	{
