...
if($spassword eq $passworddb)
{
	...
	$sth = $db->prepare("select UserName, FirstName, LastName, Email, ActiveUser, ResponseEmail,FieldofInterest, Comments,LastEmailSentOn from user where ActiveUser='active' OR ActiveUser IS NULL OR ActiveUser='' ORDER BY UserName");
	$sth->execute();
	$user_no = 0;

	# Populate users dict, prep for sorting.
	my $UserName, $FirstName, $LastName, $Email,$ActiveUser, $ResponseEmail, $FieldofInterest, $Comments, $LastEmailSentOn;
	$countusers=0;
	$countarchivedusers=0;
	$countcurrentusers=0;

	while(($UserName, $FirstName, $LastName, $Email, $ActiveUser, $ResponseEmail, $FieldofInterest, $Comments, $LastEmailSentOn) = $sth->fetchrow_array) {
		$users{$UserName} = [$LastName, $FirstName, $Email, $ActiveUser, $ResponseEmail, $FieldofInterest, $Comments, $LastEmailSentOn];
		$countusers++;
...
#1. Start of Users table 
	print "<img id ='showImg0' src='../_image/plus.gif' onmouseover=\"  if (curr==1){ this.src='../_image/plus-red.gif'; } else {this.src='../_image/minus-red.gif';}\" onmouseout=\"  if (curr==1){ this.src='../_image/plus.gif'; } else {this.src='../_image/minus.gif';} \" style='vertical-align:middle;cursor:pointer;padding:0px 15px 0px 5px' onClick ='check_button(0)' >";
...
print "<table border=\"3\" cellpadding=\"3\" cellspacing=\"3\" width=\"1070\">\n";
	print "<tr>\n";
		...
		print "<td width=\"60\"><b>Status</b></td>\n";
		print "<td width=\"60\"><b>Last Activity</b></td>\n";
	...
	foreach $UserName (sort keys %users) {
		($LastName, $FirstName, $Email,$ActiveUser,$ResponseEmail,$FieldofInterest,$Comments, $LastEmailSentOn) = @{$users{$UserName}};
...
if($ActiveUser ne 'inactive')
		...
		print "<td width=\"60\">\n";
		if($adminlogin ne "John") {
			if($UserName ne "John"){
				print "<a href=\"modifyuser.php?SID=$sid&adminlogin=$adminlogin&tag=$UserName\"> Modify</a>\n";
			} else {
				print "Modify\n";
			}
		} else {
...
#          	print "</form>\n";
		...
		if($ResponseEmail eq 'Pending') {
			print "<font color='red'><b>$ResponseEmail</b></font> </td>\n"; #<img id ='showI' src='../_image/minus.gif'>
		}
		else
			{
		if($ResponseEmail eq 'Updated') {
			print "<font color='green'><b>$ResponseEmail</b> </font></font></td>\n"; #<img id ='showI' src='../_image/plus.gif'>
		}
		else {
			print "&nbsp;</td>\n";
		}
		...
		print "<td width=\"60\">";
		if($LastEmailSentOn eq ""){
			print "&nbsp;</td>\n";
		}
		else
		{
		@ymn = split(/-/, $LastEmailSentOn);

			print "$ymn[1]-$ymn[2]-$ymn[0]</td>\n"; 
		}
		print "</tr>\n";
...
#print "<p>poppppppppppppppppppppppppppppppppppppp</p>";
...
# Dict for user info.
	...
	$sth = $db->prepare("select UserName, FirstName, LastName, Email, Position, FieldofInterest,  SubmittedDate,TransferredToUsers from UsersInQuery");
	$sth->execute();
	$user_no = 0;

	# Populate users dict, prep for sorting.
	my $UserName, $FirstName, $LastName, $Email, $FieldofInterest, $TransferredToUsers;
	$countusersinquery=0;

	while(($UserName, $FirstName, $LastName, $Email, $Position, $FieldofInterest, $SubmittedDate,$TransferredToUsers) = $sth->fetchrow_array) {
		if($TransferredToUsers ne 1)
		{
		$usersinquery{$LastName} = [$LastName, $FirstName, $Email, $Position, $FieldofInterest, $SubmittedDate,$TransferredToUsers];
		$countusersinquery++;
		}
	}
	
	print "<br/><img id ='showImg1' src='../_image/plus.gif' onmouseover=\" if (queryuser==1){ this.src='../_image/plus-red.gif' } else {this.src='../_image/minus-red.gif'}\" onmouseout=\"  if (queryuser==1){ this.src='../_image/plus.gif' } else {this.src='../_image/minus.gif'} \" style='vertical-align:middle;cursor:pointer;padding:0px 15px 0px 5px' onClick ='check_button(1)' >";
	print "<font size='4'><b>New Account Requests: $countusersinquery</b></font>\n";
	print "<div id='usersquery' style='display:none' align=\"center\"><br/>";
  	#print "<center>\n";
	print "<table border=\"3\" cellpadding=\"3\" cellspacing=\"3\" width=\"1050\">\n";
	print "<tr>\n";
#print "<script language='javascript'>alert('enter2');</script>";
		print " <td width=\"20\"><b>Select</b></td>\n";
		print "<td width=\"140\"><b>Account name</b></td>\n";
		print "<td width=\"150\"><b>Last Name</b></td>\n";
		print "<td width=\"150\"><b>First Name</b></td>\n";
    	print "<td width=\"190\"><b>Email</b></td>\n";
		print "<td width=\"100\"><b>Position</b></td>\n";
		print "<td width=\"340\"><b>Field of Interest</b></td>\n";
...
# Start of Archived Users table
	print "<img id ='showImg2' src='../_image/plus.gif' onmouseover=\" if (archiveduser==1){ this.src='../_image/plus-red.gif' } else {this.src='../_image/minus-red.gif'}\" onmouseout=\"  if (archiveduser==1){ this.src='../_image/plus.gif' } else {this.src='../_image/minus.gif'} \" \" style='vertical-align:middle;cursor:pointer;padding:0px 15px 0px 5px' onClick ='check_button(2)' >";
