...
$db= DBI->connect($dsn, "dbo260244667", "curu11i");
	...
	print "        <td width='450'>";
	print '         <input type="radio" name="uscidradiobox" id="uscid_yes" value="yes" onclick="calc(this);" >Yes
					<input type="radio" name="uscidradiobox" id="uscid_no" value="no" onclick="calc(this);" >No ';

	print "          <input type='text' name = 'uscbox' maxlength='10' id = 'uscbox' style='display:none' >";
	print "	</td>";
	print "</tr>";	
...
#	print "<input type=\"hidden\" name=\"SID\" value = \"$sid\">\n";
	...
	print " if(document.getElementById('uscid_yes').checked) {
				if(  document.getElementById('uscbox').value.length==0	){
							alert('Please enter your 10 Digit USC ID');
							return false;
				}
		}";
	...
	#		var context = canvas.getContext('2d');
		#	var img = document.getElementById('captchabox');
  				...
  				xmlhttp.onreadystatechange=function()	{	
					...
					if (xmlhttp.readyState==4 && xmlhttp.status==200) {
								...
								if(xmlhttp.responseText==0){
									...
									var answer = confirm('User with given details already Exists, Do you have an Account with CEMMA ? ');
 									if( answer == true ){
									 	flag = false;
 										sendAccountDetails(username); // email account details
 									}else{
										flag = false;
										alert('Please choose a different Login Details to create your Account');  // create account with incrementing the name + 1
									}
									flag = false;
								}else if(xmlhttp.responseText==2){
									alert('This Email Id is already registered with CEMMA');
									flag = false;
								}else if(xmlhttp.responseText==3){
									var answer = confirm('User with this First Name and Last Name already exists, Do you have an Account with CEMMA ? ');
 									if( answer == true ){
									 	flag = false;
 										sendAccountDetails(username); // email account details
 									}else{
										flag = false;
										// need to create a new user here
										//document.newuser.LastName.value = 'Chandratre';
									}
	...
	print "
		function sendAccountDetails(username){
	 		...
	 		if (window.XMLHttpRequest)	 
	 	 	 ...
	 	 	 { 	 
  				...
  				xmlhttp.open(\"GET\",\"http://cemma-usc.net/schedule/cgi-bin/sendAccountDetails.php?username=\"+username,false);
				xmlhttp.send(null);
 		}
