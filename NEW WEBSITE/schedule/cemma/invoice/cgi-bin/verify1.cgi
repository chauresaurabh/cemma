...
else
{
	...
	$db= DBI->connect("DBI:mysql:cemma","root","");
	$sth = $db->prepare("select Passwd, Class from user where UserName = \"$login\"");
	$sth->execute();
	($passworddb, $Class) = $sth->fetchrow_array();
	$sth->finish();
	$db->disconnect();
	...
	elsif($passworddb eq $password & $Class eq 1) # Verify administrator's identity
	{
		...
		print "<body onload=\"administrator.submit()\">";
	...
	elsif($passworddb eq $password)
	{
		...
		print "Content-type: text/html\r\n\r\n";
		print "<body onload=\"user.submit()\">";  #Redirect to normal user's interface, in which the person only has the access to his/her own data.
