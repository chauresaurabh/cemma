...
if($spassword eq $passworddb)
{
	...
	print "<html>\n";
	print "<head>\n";
	print "<meta http-equiv=\"refresh\"; CONTENT=\"3; url=manuser.cgi?SID=$sid&adminlogin=$adminlogin\">\n";
	print "</head>\n";
	...
	{ # change admin himself
		my $ins = "update user set Passwd='$password', Class='$class', FirstName='$firstname', LastName='$lastname', Name='$firstname $lastname', Email='$email', Telephone='$Tel', Dept='$dept', Advisor='$advisor', GradYear = $gradyear, GradTerm ='$gradterm' where UserName = '$login'";
		$db->do($ins);
