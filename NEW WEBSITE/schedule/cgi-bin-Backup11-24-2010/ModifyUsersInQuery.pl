...
if($spassword eq $passworddb)
{
	...
	$username=param("tag");
	$sth = $db->prepare("select Passwd, Class, Email, FirstName, LastName, Telephone, Dept, Advisor, GradYear, GradTerm,FieldofInterest, Comments from UsersInQuery where UserName = '$username'");
	$sth->execute();
	($password, $class, $email, $firstname, $lastname, $tel, $dept, $advisor, $gradyear, $gradterm, $FieldofInterest, $Comments) = $sth->fetchrow_array();
	$db->disconnect;	
	$sth->finish();
  ...
  foreach my $val (@values) {
	...
	print "        <td width=\"166\"> \n";
	print "<input type=\"checkbox\" name=\"fieldofinterest\" value=\"LifeScience\" $LifeSciencechecked onclick=\"checkfieldofinterest()\">Life Science";		
	print "<input type=\"checkbox\" name=\"fieldofinterest\" value=\"Physical Science\" $PhysicalSciencechecked onclick=\"checkfieldofinterest()\">Physical Science";	
	print "	</td>\n";
	print "</tr>\n";
