...
#print "$field ";
	...
	{ # change admin himself
		my $ins = "update UsersInQuery set Passwd='$password', Class='$class', FirstName='$firstname', LastName='$lastname', Name='$firstname $lastname', Email='$email', Telephone='$Tel', Dept='$dept', Advisor='$advisor', GradYear = $gradyear, GradTerm ='$gradterm', FieldofInterest='$field', Comments='$Comments' where UserName = '$login'";
		$db->do($ins);

		$db->do("delete from UsersInQuery_instr_group where Email = '$old_email'");
		...
		for ($ii=1; $ii <= $instrNum; $ii++)
		{
			...
			if ($instrNo ne "")
			{
				...
				$db->do("insert into UsersInQuery_instr_group (InstrNo, Email, InstrSigned) values('$instrNo', '$email', '$YY-$MM-$DD')");
			}
		}
	...
	{ # change other users
		...
		{
			my $ins = "insert into UsersInQuery (UserName, Passwd, Class, FirstName, LastName, Name, Email, Telephone, Dept, Advisor, GradYear, GradTerm) values ('$login', '$password', '$class', '$firstname', $lastname', '$firstname $lastname', '$email', '$tel','$dept', '$advisor', $gradyear, '$gradterm')";

			$db->do("delete from UsersInQuery where UserName='$tag'");
			$db->do($ins);
