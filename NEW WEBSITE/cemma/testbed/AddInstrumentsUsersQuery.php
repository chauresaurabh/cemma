<form id="myForm" name="myForm" action="AddInstrumentsUsersQuery.php" style="11px/1.6em Tahoma,Geneva,sans-serif">
...
<table width=\"250\">
				...
				$sql3 = "UPDATE UsersInQuery SET Passwd = '$password', Email='$Email',Class='$adminclass', FirstName='$FirstName', LastName='$LastName',Name = '".$FirstName." ".$LastName."', Telephone='$Telephone', Dept='$Dept', Advisor='$Advisor',GradYear='$GradYear', Position='$position', FieldofInterest ='$fieldofinteresttosave',  Comments='$Comments', AccountNum='$AccountNum'  WHERE UserName = '$username'";
		
		#echo $sql3;
		mysql_query($sql3) or die( "An error has occurred in query1: " .mysql_error (). ":" .mysql_errno ()); 

		$sql123 = "DELETE FROM UsersInQuery_instr_group WHERE Email='$Email'  ";  
	 ...
	 for ($i=0;$i<$countInstrumentName ; $i++)
				...
				$datetoput=$yy."-".$mm."-".$dd;
 							include_once(DOCUMENT_ROOT."includes/DatabaseOld.php");
			$sql3 = "INSERT INTO UsersInQuery_instr_group (InstrNo,  Email , InstrSigned ) VALUES ('$instrno', '$Email', 		'$datetoput' )";  
			#echo $permission[$k]."<br>";
			#echo $sql3."<br>";
			mysql_query($sql3) or die( "An error has occurred in query2: " .mysql_error (). ":" .mysql_errno ()); 
							...
							include_once(DOCUMENT_ROOT."includes/DatabaseOld.php");
							    $userId = $_GET['userId'];
 							$sql1 = "select * from UsersInQuery_instr_group where Email ='".$userId."'";
