...
<script type="text/javascript">
	...
	function initializeData(){
		...
		if(document.getElementById("CustomersName").options[i-1].value == "<?php echo $row['CemmaStaffMember'] ?>"){
		...
		if(<?php echo $row['WithOperator']?>== 1) {
			...
			document.getElementById("CemmaStaff").style.visibility="visible";
			  ...
			  document.getElementById("CemmaStaff").options[i++]=new Option("<? echo $row2['Name'];?>", "<? echo $row2['Name'] ;?>");
					if(document.getElementById("CemmaStaff").options[i-1].value == "<?php echo $row['Name'] ?>"){
		...
		function changeRecord() {
			...
			var CemmaStaff = e2.options[e2.selectedIndex].value;
